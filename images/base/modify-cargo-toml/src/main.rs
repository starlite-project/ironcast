...
fn main() {
	...
	let (input_filename, output_filename): (PathBuf, PathBuf) = {
		let input = args.next().unwrap_or_else(|| OsString::from("Cargo.toml"));
		let output = args.next().unwrap_or_else(|| input.clone()).into();
